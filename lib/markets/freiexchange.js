...
function get_summary(coin, exchange, cb) {
  ...
  request({uri: url, json: true}, function (error, response, body) {
    if (error) {
      ...
      summary['ask'] = parseFloat(body['ANC_BTC'][0]['lowestSell']).toFixed(8);
      summary['bid'] = parseFloat(body['ANC_BTC'][0]['highestBuy']).toFixed(8);
      summary['volume'] = parseFloat(body['ANC_BTC'][0]['volume24h']).toFixed(8);
      summary['volume_btc'] = parseFloat(body['ANC_BTC'][0]['volume24h_btc']).toFixed(8);
      summary['high'] = parseFloat(body['ANC_BTC'][0]['high']).toFixed(8);
      summary['low'] = parseFloat(body['ANC_BTC'][0]['low']).toFixed(8);
      summary['last'] = parseFloat(body['ANC_BTC'][0]['last']).toFixed(8);
      summary['change'] = parseFloat(body['ANC_BTC'][0]['percent_change_24h']);
